    ...
    for (i = 0; i < nframes; ++i)
    {
        ...
        if (eventIndex < eventCount)
        {
            ...
            case 0xe0:
                {
                // Pitch Bend
                unsigned int pb = inEvent.buffer[1] + inEvent.buffer[2] * 128;
                cout << "Pitch Bend = " << pb << endl;
                SyntheSound::setPitchBend(pb);
                }
                break;
            case 0xf0:
                // Le cas f0 est particulier puisque c'est une liste de 16 commandes
                // indépendante du canal
                // L'utilisation du canal déterminera précisément la commande à réaliser
                //cout << "Une commande temps réels" << endl;
                break;
            default:
