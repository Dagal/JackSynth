...
int process(jack_nframes_t nframes, void* arg)
{
  ...
  for (i = 0; i < nframes; ++i)
      ...
      if (eventIndex < eventCount)
	{
	  ...
	  switch (commandeEvent)
		  ...
		  (!syntheSounds.empty()))
		{
		  ...
		  // et puis seulement comparer les valeurs
		  ...
		  for (list<SyntheSound*>::iterator it = syntheSounds.begin(); it != syntheSounds.end(); ++it)
		    {
		      ...
		      if ((*it)->note() == inEvent.buffer[1])
			{
			  cout << "On a trouvé une note." << endl;
			  // Si oui, on l'efface
			  delete *it;
			  cout << "Elle est effacée." << endl;
			  // et on la retire de la liste
			  syntheSounds.erase(it);
			  cout << "Elle est retirée de la liste." << endl;
			  // On sort directement de la boucle, notre travail est fait.
			  break;
			}
		    }
		  cout << "Effacement terminé." << endl;
		}
	      break;
	    case 0x90:
	      // NoteOn
	      ...
	      if ((canal == 0) || (canalEvent == canal))
		{
		  ...
		  if (syntheSounds.size() < maxSound)
			...
			{
			  // Sinon faut vérifier si ce son existe.
			  ...
			  // Si le n'est pas trouvé alors on le crée.
			  if (!isNoteExist) syntheSounds.push_back(new SyntheSound(inEvent.buffer[1], inEvent.buffer[2]));
			}
		    }
		}
	      break;
      ...
      if (!syntheSounds.empty())
	{
	  ...
	  for (list<SyntheSound*>::iterator it = syntheSounds.begin(); it != syntheSounds.end(); ++it)
	    {
	      currentAngle = (*it)->getAngle();
	      
	      // Il faut réduire à une valeur inférieure à 6.28
	      // sinon la valeur devient vite trop grande
	      // et le son commence à faire n'importe quoi.
	      if (currentAngle >= 2 * 3.1415926353) currentAngle -= 2 * 3.1415926353;
	      // Valeur de base du son
	      baseSound +=
		volumeGeneral / 127.0 *
		(*it)->velocite() / 100.0 *
		sin(currentAngle);
	    }
	  baseSound /= syntheSounds.size();
	  // Mise à jour des buffers de sortie.
	  outBufferLeft[i] =
	    baseSound *
	    ((pan>=64)?((127-pan)/63.0):1);
	  outBufferRight[i] =
	    baseSound *
	    ((pan<=64)?(pan/64.0):1);
	}
